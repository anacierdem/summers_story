...
/**
 * @brief Helpers to write binary big-endian data to a file
 */
...
#include "../libdragon/include/GL/gl_enums.h"
...
terrain_t* terrain_alloc()
{
    terrain_t *model = calloc(1, sizeof(terrain_t));
    model->magic = TERRAIN_MAGIC;
    model->version = TERRAIN_VERSION;
...
void terrain_write(terrain_t *model, FILE *out)
{
    // Write header
    ...
    int heightmap_placeholder = w32_placeholder(out);
    int heightmap_indices_placeholder = w32_placeholder(out);
    ...
    for (size_t i = 0; i < model->num_meshes; i++)
    {
        ...
        for (size_t j = 0; j < TERRAIN_MESH_NAME_SIZE; j++) {
            w8(out, mesh->name[j]);
        }
        for (size_t j = 0; j < 16; j++) {
            wf32(out, mesh->transform[j]);
        }
    ...
    // Write heightmap
    walign(out, 4);
    uint32_t offset_heightmap = ftell(out);
    int num_samples = (TERRAIN_GRID_SIZE * TERRAIN_GRID_SIZE);
    for (size_t i = 0; i < num_samples; i++)
    {
        wf32(out, model->heightmap[i]);
    }

    walign(out, 4);
    uint32_t offset_heightmap_indices = ftell(out);
    for (size_t i = 0; i < num_samples; i++)
    {
        wf32(out, model->heightmap_indices[i]);
    }
    ...
    w32_at(out, heightmap_placeholder, offset_heightmap);
    w32_at(out, heightmap_indices_placeholder, offset_heightmap_indices);
...
int convert_mesh(cgltf_node *in_node, mesh_t *out_mesh)
{
    strncpy(out_mesh->name, in_node->mesh->name, TERRAIN_MESH_NAME_SIZE);
    out_mesh->name[TERRAIN_MESH_NAME_SIZE - 1] = 0;

    // Convert transform
    cgltf_float matrix[16] = {0};
    cgltf_node_transform_world(in_node, matrix);

    _Static_assert(
    sizeof(matrix) == sizeof(out_mesh->transform),
    "mesh_t transform incorrectly sized");
    memcpy(out_mesh->transform, matrix, sizeof(matrix));
    // Convert primitives
    out_mesh->num_primitives = in_node->mesh->primitives_count;
    out_mesh->primitives = calloc(in_node->mesh->primitives_count, sizeof(primitive_t));
    for (size_t i = 0; i < in_node->mesh->primitives_count; i++)
    {
        ...
        if (convert_primitive(&in_node->mesh->primitives[i], &out_mesh->primitives[i]) != 0) {
...
int convert_heightmap(cgltf_mesh *in_mesh, terrain_t *out_terrain)
{
    assert(in_mesh->primitives_count >= 1);

    cgltf_primitive *first_primitive = in_mesh->primitives;

    cgltf_attribute *position = NULL;
    for (size_t i = 0; i < first_primitive->attributes_count; i++)
    {
    ...
    int num_vertices = position->data->count;
    assert(num_vertices > 0);

    if (flag_verbose) {
        printf("no vert: %u\n", num_vertices);
        printf("no ind: %lu\n", first_primitive->indices->count);
    }

    size_t num_components = cgltf_num_components(position->data->type);
    assert(num_components == 3);


    size_t num_values = num_components * position->data->count;
    float *temp_buffer = malloc(sizeof(float) * num_values);

    if (cgltf_accessor_unpack_floats(position->data, temp_buffer, num_values) == 0) {
        fprintf(stderr, "Error: failed reading attribute data\n");
        free(temp_buffer);
        return 1;
    }

    // Assume the terrain mesh is grid aligned and matches the grid size
    // Here we find the correspondance between mesh vertices and grid points
    // The grid will be used to biliinerly interpolate the height of the terrain
    // And also to update the terrain mesh with the new heights.
    // Other assumptions are that it is square and centered at the origin
    // The grid is oriented such that the first axis (x) is along positive x
    // and the second axis (y) is along positive z

    // TODO: get this from the file
    float terrain_step = 2 * TERRAIN_HALF_WIDTH / (TERRAIN_GRID_SIZE - 1);
    ...
    out_terrain->heightmap = calloc(TERRAIN_GRID_SIZE * TERRAIN_GRID_SIZE, sizeof(float));
    out_terrain->heightmap_indices = calloc(TERRAIN_GRID_SIZE * TERRAIN_GRID_SIZE, sizeof(int));

    for (int i = 0; i < num_vertices; i++){
        float vertex_x = temp_buffer[0 + i * 3];
        float vertex_y = temp_buffer[1 + i * 3];
        float vertex_z = temp_buffer[2 + i * 3];

        int x = (vertex_x / terrain_step) + ((TERRAIN_GRID_SIZE - 1)/2);
        int y = (vertex_z / terrain_step) + ((TERRAIN_GRID_SIZE - 1)/2);

        if (flag_verbose) {
            printf("vertex id: %u ", i);
            printf("x: %f ", vertex_x);
            printf("y: %f ", vertex_y);
            printf("z: %f\n", vertex_z);
        }

        // Make sure we have an aligned grid
        float remainder_x = fmodf(vertex_x, terrain_step);
        float remainder_y = fmodf(vertex_z, terrain_step);
        assert(remainder_x < 0.1f);
        assert(remainder_y < 0.1f);

        int index = y * TERRAIN_GRID_SIZE + x;

        out_terrain->heightmap_indices[index] = i;
        out_terrain->heightmap[index] = vertex_y;
    }
    ...
    if (flag_verbose) {
        for (int y=0; y < TERRAIN_GRID_SIZE; y++) {
            for (int x=0; x < TERRAIN_GRID_SIZE; x++) {
                int index = y * TERRAIN_GRID_SIZE + x;
                printf("%0.2f ", out_terrain->heightmap[index]);
            }
            printf("\n ");
        }
    }
...
int convert(const char *infn, const char *outfn)
{
    ...
    for (size_t i = 0; i < data->nodes_count; i++) {
        ...
        if (!strcmp(mesh->name, "__terrain")) {
            assert(!terrain_found);
            terrain_found = true;
            if (flag_verbose) {
                if (mesh->name != NULL) {
            ...
            if (convert_heightmap(mesh, model) != 0) {
                if (mesh->name != NULL) {
                    fprintf(stderr, "Error: failed converting heightmap %s\n", mesh->name);
                } else {
                    fprintf(stderr, "Error: failed converting heightmap %zd\n", i);
                }
                goto error;
            }
            if (convert_mesh(&data->nodes[i], &model->meshes[0]) != 0) {
                fprintf(stderr, "Error: failed converting terrain mesh %s\n", mesh->name);
                goto error;
            }
        } else {
            if (convert_mesh(&data->nodes[i], &model->meshes[terrain_found ? i : (i+1)]) != 0) {
                if (mesh->name != NULL) {
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        asprintf(&outfn, "%s/%s.terrain", outdir, basename_noext);
        if (flag_verbose)
